...
public class PayRecordItemProcessor implements ItemProcessor<PayRecord, PayRecord> {
    private static final Logger log = LoggerFactory.getLogger(BlackListFieldSetMapper.class);

    // 数据处理
    public PayRecord process(PayRecord payRecord) throws Exception {
        String threadName = Thread.currentThread().getName();
        log.info("线程{}->开始数据处理...",threadName);
        payRecord.setId(UUID.randomUUID().toString());
        log.info("线程{}->处理中数据：{}",threadName, payRecord.toString());
        return payRecord;
    }
