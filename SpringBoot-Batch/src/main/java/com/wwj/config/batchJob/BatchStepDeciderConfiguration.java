...
import com.wwj.config.batchJob.decider.MyJobExecutionDecider;
...
public class BatchStepDeciderConfiguration {

    private static final Logger logger = LoggerFactory.getLogger(BatchStepDeciderConfiguration.class);
    ...
    public Job importFileJob() throws IOException {
        JobExecutionDecider decider = new MyJobExecutionDecider();
        return jobs.get("deciderJob")
                .incrementer(new RunIdIncrementer())//添加一个自增的值，以区分不同的Job实例，当然，这个值在job的其他的地方并不会用到，仅仅是为了标示不同JobInstance
                .listener(jobCompletionNotificationListener)
                .start(readFileStep)
                //分支流程控制
                .next(decider).on("FAILED").to(readFileStep)
                .on("COMPLETED").to(readFileStep)
                .end()
                .build();
    }
     ...
     * @throws IOException
     */
    @Bean(name = "deciderJob-step1-read")
    @StepScope
     ...
     * @return
     */
    @Bean(name = "deciderJob-step1-processor")
    @StepScope
     ...
     * @return
     */
    @Bean(name = "deciderJob-step1-writer")
    @StepScope
